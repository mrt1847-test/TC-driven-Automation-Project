...
- **High — Stop buttons are DB-only state changes:** Webwright run cancel and execution cancel endpoints mark rows `cancelled`, but the running subprocess is not tracked/killed and may later overwrite status to completed/failed. Existing rows **C3-09** and **C9-08** must include a process registry/cancellation token, status race prevention, log-stream closure, and tests proving the child process actually exits.
...
- [ ] **C3-09** Webwright subprocess cancel — §7.3 | Phase 5 | Layer: Worker | Depends: C3-02, C4-04 | Spec: API_SPEC (replace DB-only cancel with real process cancellation: maintain a per-run/job process registry keyed by `WebwrightRun.id` and returned `jobId`, expose cancellation that sends graceful terminate then kill after timeout, stop heartbeat/pipe tasks, write masked cancellation diagnostics to stdout/stderr artifacts, close or mark the log stream, prevent the background task from overwriting `cancelled` with `completed`/`failed`, and add tests with a fake long-running Webwright process proving the child exits, run/case status remain cancelled, artifacts are harvested, and retry creates a fresh run)
...
- [ ] **C7-16** selector candidate ranking in body plans — Spec: STRUCTURING_SPEC, SELF_HEALING_SPEC | Phase 5 | Layer: Worker | Depends: C12-02, C7-11 (apply a selector preference policy before writing `body_plan_json`: load persisted `SelectorCandidate` rows by `sourceRawActionId`, rank by semantic stability and confidence — prefer `test_id`, then role/accessibility, then stable text, then CSS, with XPath last unless no alternative exists — emit the chosen selector into the plan, store chosen/runner-up candidate IDs and confidence in plan metadata for review/healing, preserve raw selector fallback when candidates are missing, and add tests for test-id extracted from CSS, role vs text ties, low-confidence fallback, candidate provenance in metadata, and generated code using the ranked selector)
...
- [ ] **C9-08** execution subprocess cancel — §7.6 | Phase 5 | Layer: Worker | Depends: C9-05, C9-01 | Spec: API_SPEC (replace DB-only cancel with real `runner.cli` cancellation: track active execution/rerun subprocesses by `ExecutionRun.id` and returned `jobId`, support graceful terminate then kill after timeout on Windows/native/WSL-compatible process trees, persist `cancelled` without later overwrite by the background task, write masked cancellation stdout/stderr/results artifacts when possible, close or mark the log stream, ensure `rerun-failed` cancellation uses the same registry, and add tests with fake long-running runner commands proving the process exits, status remains `cancelled`, no stale `ExecutionResult` rows are persisted, and a subsequent rerun uses a fresh run directory)
