...
- **`RuntimeProfile` 해석** — `settings.runtime` + `TC_STUDIO_*` 환경 변수로 Worker/Webwright/Runner가 동일 Python·browser cache 사용 ([docs/RUNTIME_SPEC.md](docs/RUNTIME_SPEC.md))
- **Webwright live readiness gate** — `base.yaml` 존재만으로 live 준비를 인정하지 않는다. `RuntimeProfile.webwright_python -m webwright.run.cli` import/실행 가능성, Webwright config, Playwright package/browser cache를 검증하고 실패 시 명시적 mock/dev mode로 분리한다.

### RuntimeProfile (2026-06-02)

| Mode | 용도 |
|------|------|
| `bundled` | Installer `resources/runtime` (embeddable Python, webwright root, template, ms-playwright) |
| `custom` | 개발/고급: `webwright.root`, `webwright.python`, `playwrightBrowsersPath` 수동 지정 |

구조화 codegen은 mapping 직접 출력이 아니라 `StructuredFlow` / `PageObjectMethod` DB → `project_generator` 경로를 사용한다 ([docs/STRUCTURING_SPEC.md](docs/STRUCTURING_SPEC.md)).
