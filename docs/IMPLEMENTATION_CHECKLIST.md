...
- **Resolved — POM method identity:** same-named steps no longer share a single `GeneratedPage` method; C7-15 scopes generated method names by case/step and repairs legacy shared POMs during selected raw refresh before optional page segmentation (**C7-17**).
...
- [ ] **C1-10** project-unique `automation_key` policy — §5.5, §6.2, §7.2 | Phase 5 | Layer: Worker | Depends: A2-02, C1-03, C1-04 | Spec: DB_SCHEMA, GENERATED_PROJECT_SPEC (make `automation_key` a true project-scoped identity before generation/export: normalize explicit import keys the same way generated keys are normalized, reject or auto-suffix duplicates within a single import batch and against existing active cases, add a DB-level uniqueness guard for active project/key rows or an equivalent pre-migration-safe validation layer, and update Excel/testrail-clone import tests to cover duplicate explicit keys, duplicate generated keys, retired/deleted case behavior, generated file path uniqueness, `mappings/cases.yaml` merge behavior, and export validation when duplicate keys are attempted)
